...
## ✅ OPS-2202-R — My own fix introduced a regression (found, then fixed)
...
- **A — Action:** **At the time: none** — a fix could not be verified within the deadline, and shipping an unverified fix is what this lab spent two incidents learning not to do. **Subsequently fixed** (`d10f8b6`): the 500 ms `notifyBedRegistry` call was moved out of the transaction, taking timeouts **89 → 0 at the unchanged pool of 25**. The recommended revert to `MYSQL_POOL_SIZE=8` was deliberately **not** carried out — it treated the symptom. See scar **OPS-2203**.
